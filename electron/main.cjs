...
ipcMain.handle('codex:run', async (_, options) => {
  ...
  const prompt = [
    ...
    'Prefer editing existing project files in place. Create new permanent files or folders only when the user explicitly asks for them or they are strictly required for the requested feature.',
    'Do not create extra sample, demo, scratch, temporary, or test files and folders just to validate work. Use the operating-system temporary directory when temporary artifacts are unavoidable, and remove every temporary artifact before finishing.',
    'Never delete or overwrite pre-existing user files merely because they look temporary or test-related. Run the project existing checks when available, and add durable tests only when the user asks for tests.',
